...
## 日记与个人日程
- ✅ 首页右侧「日记」面板：按月日历 + 当日日记卡片 + 当日个人日程联动
- ✅ 日历状态点：已写日记圆点（日期上方）/ 日程状态点（下方：逾期红 / 今天绿 / 未来蓝 / 已完成灰）+ 图例
...
- ✅ 「看日记」书页式浏览：左右双页展开（左旧右新）、跳过无日记日期；打开定位**本月第一篇**（本月无则落到最近有日记月份），顶栏**年月下拉**跨月直跳；留白处为装饰「扉页」（圆环羽毛笔 + 年月大字，左「新的一月」/ 右「本月终章」）；箭头 / 方向键整组翻页、仅展示只读
- ✅ 个人日程管理：添加 / 勾选完成 / 双击编辑（回车保存 / Esc 取消）/ 删除，完成进度 x/y
- ✅ 本地 SQLite 存储：`diaries`（每天至多一篇，`diary_date` 唯一）、`schedules`（某天多条）

> 使用说明见 [日记与个人日程](user-guide/diary)。
...
- ✅ 内置插件引导 `plugins/bootstrap.ts`：首个 home-header 插件「英语字典·生词本」（v1.4.0）
...
- ✅ 窗口状态通过 Jotai 原子（前五种）与插件模块状态（英语字典）跨页面共享，Tauri 事件跨窗口同步
