...
description: MirageInk (TimeWrite/智写时光) 项目版本发布与 Git 工作流技能。当用户需要发布 GitHub Releases、更新版本号、bump 版本、提交代码到仓库、推送代码到远程仓库、暂存修改并切分支提交、或对项目进行发版时使用。触发词包括：发布大版本、发布新功能版本、发布优化版本、发布补丁版本、重置当前版本、提交代码到仓库、推送到远程仓库、暂存修改、切新分支、stash、新建分支提交、发版、bump version、release、tag、commit、push 等。
---

# MirageInk (TimeWrite) 版本发布

## Overview

该技能负责 MirageInk / TimeWrite（Tauri v2 + React 19 + TypeScript）项目的版本号管理和 GitHub Releases 发布流程。版本号在项目中分布于 **5 个文件**，必须全部同步更新。

## 项目信息

| 项目 | 值 |
|------|------|
| 项目名称 | MirageInk (TimeWrite / 智写时光) |
...
| 暂存当前修改并新增分支提交 | stash → 新建分支 → stash pop → commit | `dev`（默认） | 将未提交的修改搬到新分支并提交 |
...
> `pnpm check` 包含 **TypeScript 类型检查**（`tsc --noEmit`）+ **Rust 编译检查**（`cargo check`），能在本地提前发现前后端编译错误，避免 CI 构建失败。

```bash
# 在 dev 分支上，先确保代码质量通过
pnpm check
# 必须全部 ✅ 通过，exit code = 0，否则禁止继续发版
# 若 Rust 工具链未安装，cargo check 会作为警告跳过（发版前建议安装 Rust 完整验证）
```
...
> 📍 **执行分支**：`main`
> 包含 tsc --noEmit + cargo check + 文件一致性，确保版本号更新后项目完整可构建。
...
### Step 8：暂存当前修改并增量分支提交

> 📍 **执行分支**：任意分支（通常在 `dev` 或功能分支）
>
> 适用场景：当前分支有未提交的修改，但需要将这些修改搬迁到一个新建分支上进行提交。常见于：
> - 写到一半发现应该在单独的功能分支上开发
> - 多个独立功能混在一起，需要拆分到不同分支
> - 当前分支不宜直接提交（如误在 `main` 上做了修改）

**⚠️ 注意**：Stash 不会保存未跟踪的新文件（untracked files）。如有新文件，需先确认是否需要一并搬迁。
...
```bash
# 暂存所有修改（含已跟踪文件的变更），并添加描述信息
git stash push -u -m "WIP: <简短描述当前修改内容>"

# 验证 stash 已保存
git stash list
```

> `-u` 参数同时暂存未跟踪的新文件（untracked files），确保所有工作不会丢失。

#### 8.3 确定新分支名称

**询问用户**新分支的名称。如用户未明确指定，则：

1. **分析暂存内容**，理解修改意图：
   ```bash
   git stash show -p stash@{0} --stat    # 查看 stash 的文件列表
   git stash show -p stash@{0}           # 查看 stash 的具体 diff
   ```

2. **根据提交类型推荐分支名**（遵循 `<type>/<简短描述>` 格式）：
   
   | 变更类型 | 分支名前缀 | 示例 |
   |---------|-----------|------|
   | 新功能 | `feat/` | `feat/article-export` |
   | Bug 修复 | `fix/` | `fix/editor-crash` |
   | 重构 | `refactor/` | `refactor/ai-panel` |
   | 性能优化 | `perf/` | `perf/markdown-render` |
...
> 如果 `stash pop` 产生冲突，需先解决冲突再继续。冲突解决后执行 `git stash drop` 清理。

#### 8.5 提交到新分支

参考 [Step 7 的提交规范](#step-7提交代码到仓库仅本地提交)，分析 diff 并生成详细 commit message：

```bash
# 暂存所有变更
git add -A

# 生成并提交（禁止泛化 commit message）
git commit -m "<type>: <简要标题>" -m "<详细正文>"
```
...
```bash
# 推送新分支到远端并设置 upstream
git push -u origin <分支名>
```

> 仅当用户明确要求或确认需要推送时才执行此步。
...
# 2. 暂存当前所有修改
git stash push -u -m "WIP: 文章导出功能"

# 3. 创建新功能分支
git checkout -b feat/article-export
...
# 5. 提交到新分支
git add -A
git commit -m "feat: 新增文章导出 PDF 功能" -m "...详细正文..."

# 6. 推送新分支到远端
git push -u origin feat/article-export
```
