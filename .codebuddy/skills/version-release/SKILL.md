...
> **注意**：更新日志条目独立存放在 `CHANGELOG.md`，由 Step 2 手动生成插入，不在 `bump_version.py` 自动更新范围内。

脚本执行后会显示更新结果和后续步骤提示。

### Step 2：自动生成 CHANGELOG 更新日志

基于 git log 自动生成更新日志条目，添加到 `CHANGELOG.md` 的 `# 更新日志` 部分顶部：
...
3. **生成更新日志条目并插入 CHANGELOG.md**，格式为：
