...
该技能负责 TimeWrite（Tauri v2 + React + TypeScript）项目的版本号管理和 GitHub Releases 发布流程。版本号在项目中分布于 5 个文件，必须全部同步更新。
...
- `README.md`（应用信息表格中的版本号，不含更新日志）
- `.github/workflows/release.yml`（workflow_dispatch 默认值）

> **注意**：更新日志条目独立存放在 `docs/CHANGELOG.md`，由 Step 2 手动生成插入，不在 `bump_version.py` 自动更新范围内。

脚本执行后会显示更新结果和后续步骤提示。

### Step 2：自动生成 CHANGELOG 更新日志

基于 git log 自动生成更新日志条目，添加到 `docs/CHANGELOG.md` 的 `# 更新日志` 部分顶部：
...
3. **生成更新日志条目并插入 docs/CHANGELOG.md**，格式为：
